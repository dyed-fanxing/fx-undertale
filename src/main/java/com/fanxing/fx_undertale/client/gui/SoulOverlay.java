...
public class SoulOverlay {
    ...
    public static final ResourceLocation[] RED = new ResourceLocation[]{
            ResourceLocation.fromNamespaceAndPath(FxUndertale.MOD_ID, "hud/soul_outline_red"),
            ResourceLocation.fromNamespaceAndPath(FxUndertale.MOD_ID, "hud/soul_fill_red")
    };
    public static final ResourceLocation[] BLUE = new ResourceLocation[]{
            ResourceLocation.fromNamespaceAndPath(FxUndertale.MOD_ID, "hud/soul_outline_blue"),
            ResourceLocation.fromNamespaceAndPath(FxUndertale.MOD_ID, "hud/soul_fill_blue")
    };
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        if (fillHeight > 0) {
            guiGraphics.blitSprite(
                    soulTexture[1],           // 纹理
                    ICON_SIZE, ICON_SIZE,     // 纹理总宽度和高度 (16x16)
                    0, ICON_SIZE - fillHeight, // 纹理起始坐标 (从底部开始)
                    0, 0,                     // 屏幕目标位置 (在变换后的坐标系中)
                    0,                        // Z层级
                    ICON_SIZE, fillHeight     // 绘制宽度和高度
            );
        }
        // 参数：纹理,x,y,z,width,height    x,y偏移,其中z指层级
        guiGraphics.blitSprite(soulTexture[0],0, 0,0,ICON_SIZE, ICON_SIZE);
        poseStack.popPose();
