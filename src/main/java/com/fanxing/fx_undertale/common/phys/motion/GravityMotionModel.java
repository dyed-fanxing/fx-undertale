 ...
 */
public class GravityMotionModel extends PhysicsMotionModel {
    private static final Logger log = LoggerFactory.getLogger(GravityMotionModel.class);
     ...
     */
    public GravityMotionModel(float strength, float softening) {
     ...
     */
    public GravityMotionModel(float baseStrength, float baseDis, float actualDis, float softening) {
        this.strength = baseStrength * (actualDis / baseDis); // 线性比例，也可改为平方
        this.softening = softening;
    }

    public GravityMotionModel(CompoundTag tag) {
        this.strength = tag.getFloat("strength");
        this.damping = tag.getFloat("damping");
        this.softening = tag.getFloat("softening");
    }

    public GravityMotionModel(RegistryFriendlyByteBuf buf) {
