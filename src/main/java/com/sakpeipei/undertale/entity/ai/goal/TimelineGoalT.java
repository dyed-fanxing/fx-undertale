...
import com.sakpeipei.undertale.common.anim.TimelineAnimT;
 ...
 * 时间线动画执行器
 */
public abstract class TimelineGoalT<T,R extends Mob & IAnimatable> extends Goal {
    private static final Logger log = LogManager.getLogger(TimelineGoalT.class);
    protected final R mob;
    protected int tick;             // 动画tick
    protected int cooldownEndTick;  // 动画结束冷却Tick点

    protected int step;             // 当前步骤索引
    protected TimelineAnimT<T> anim;

    public TimelineGoalT(R mob) {
        this.mob = mob;
    }
    ...
    @NotNull
    protected abstract TimelineAnimT<T> select(LivingEntity target);
